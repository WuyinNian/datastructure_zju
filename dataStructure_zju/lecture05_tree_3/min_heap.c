...
ElementType deleteMin(MinHeap minH)
{
    ...
    for (parent = 1; 2 * parent <= minH->size; parent = child)
    {
        ...
        if (child != minH->size && minH->data[child] > minH->data[child + 1]) //判断左儿子是否是最后一个元素 并且找出最小的儿子
            child++;
        if (temp <= minH->data[child]) //要放置的元素如果小于最小的儿子，说明该parent是该元素要放置的位置
            break;
